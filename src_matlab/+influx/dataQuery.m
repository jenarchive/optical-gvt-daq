function data = dataQuery(measurement,run,scan,opts)
% DATAQUERY API to get influx data for a specific scan
arguments
    measurement string
    run double
    scan double
...
sprintf('|> filter(fn: (r) => r["ScanNum"] == "%.0f")',scan),...
'|> filter(fn: (r) => r["IsTesting"] == "1")',...
'|> pivot(rowKey: ["_time"], columnKey: ["_field"], valueColumn: "_value")',...
'|> map(fn: (r) => ({r with unix_time: uint(v: r._time)}))',...
'|> drop(columns: ["_start","_stop","host","IsTesting","PolarNum","RunNum","ScanNum","_measurement"])',...
'|> yield()'];
